...
class SHAC(BaseAlgorithm):
    ...
    def __init__(self, env: types.Env, config: dict, algo_key: types.PRNGKey):
        ...
        self.actor_grad_fn = jax.grad(actor_loss_fn, has_aux=True)
    ...
    def _actor_loss_fn(
        ...
        def env_step_fn(carry: Tuple[types.State, types.PRNGKey], _):
            ...
            return (next_state, next_key), (next_state.reward, transition)
        # rollout short horizon steps
        (final_state, _), (rewards, transitions) = jax.lax.scan(
        ...
        return actor_loss, {
            ...
            "final_state": jax.lax.stop_gradient(final_state),  # stop_gradient: gradient of reward-state and cost-state only count to this step
    ...
    def _update(
        ...
        actor_grad, info = self.actor_grad_fn(
        ...
        #         actor_grad, is_leaf=lambda x: isinstance(x, jax.ShapedArray)
        #     ) 
        ...
        (
            ...
            actor_grad, training_state.actor_optimizer_state
        )
        flatten_grad = jax.tree_util.tree_leaves(
                actor_params_update, is_leaf=lambda x: isinstance(x, jax.ShapedArray)
            ) 
        flatten_grad = jnp.concatenate([x.reshape(-1, 1) for x in flatten_grad])

        jax.debug.print("update grad normal:{}", jp.norm(flatten_grad))
