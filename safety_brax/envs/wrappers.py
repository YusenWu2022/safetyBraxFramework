...
from safety_brax.envs import env as brax_env
import dm_env
...
class VectorWrapper(brax_env.Wrapper):
    ...
    def __init__(self, env: brax_env.Env, num_local_envs: int):
        super().__init__(env)
        self.num_local_envs = num_local_envs

    def reset(self, rng: jp.ndarray) -> brax_env.State:
        assert rng.shape == (2,), "rng must be a single key"
        rng = jp.random_split(rng, self.num_local_envs)
...
class EpisodeWrapper(brax_env.Wrapper):
    ...
    def step(self, state: brax_env.State, action: jp.ndarray) -> brax_env.State:
        def f(state, _):
            ...
            return nstate, [nstate.reward, nstate.cost]

        state, [rewards, costs] = jp.scan(f, state, (), self.action_repeat)
        state = state.replace(
            reward=jp.sum(rewards, axis=0),
            cost=jp.amax(costs, axis=0),
        )  # Use the maximum value of cost to represent the cost of repeated actions
...
class AutoResetWrapper(brax_env.Wrapper):
    ...
    def step(self, state: brax_env.State, action: jp.ndarray) -> brax_env.State:
        ...
        return state.replace(qp=qp, obs=obs)  # TODO: reset to a different state
...
class EvalWrapper(brax_env.Wrapper):
    ...
    def reset(self, rng: jp.ndarray) -> brax_env.State:
        ...
        reset_state.metrics["cost"] = reset_state.cost
    ...
    def step(self, state: brax_env.State, action: jp.ndarray) -> brax_env.State:
        ...
        nstate.metrics["cost"] = nstate.cost
...
class GymWrapper(gym.Env):
    ...
    def render(self, mode="human"):
        ...
        from safety_brax.engine.io import image
