...
def rodrigues_rotation_vec_to_R(v):
  theta = jp.norm(v)
  r = jp.array(v).reshape(3, 1) / theta

  def rodrigues_rotation(r, theta):
      r = jp.array(r).reshape(3, 1)
      rx, ry, rz = r[:, 0]
      M = jp.array([
          [0, -rz, ry],
          [rz, 0, -rx],
      ...
      R = jp.eye(3)
      R_ = (jp.cos(theta) * jp.eye(3) +        \
                  (1 - jp.cos(theta)) * r @ r.T +    \
                  jp.sin(theta) * M)
      return R_
...
def eulerAngles2rotationMat(theta, format='degree'):
    ...
    R_x = jp.array([[1., 0., 0.],
                    [0., jp.cos(theta[0]), -jp.sin(theta[0])],
                    [0., jp.sin(theta[0]), jp.cos(theta[0])]
                    ])
 
    R_y = jp.array([[jp.cos(theta[1]), 0., jp.sin(theta[1])],
                    [0., 1., 0.],
                    [-jp.sin(theta[1]), 0., jp.cos(theta[1])]
                    ])
 
    R_z = jp.array([[jp.cos(theta[2]), -jp.sin(theta[2]), 0.],
                    [jp.sin(theta[2]), jp.cos(theta[2]), 0.],
                    [0., 0., 1.]
                    ])
    R = jp.dot(R_z, jp.dot(R_y, R_x))
    return R

# Checks if a matrix is a valid rotation matrix.
def isRotationMatrix(R) :
    Rt = R.transpose()
    shouldBeIdentity = jp.dot(Rt, R)
    I = jp.eye(3)
    n = jp.norm(I - shouldBeIdentity)
    return n < 1e-6
...
def rotationMatrixToEulerAngles(R) :
    ...
    sy = jp.sqrt(R[0,0] * R[0,0] +  R[1,0] * R[1,0])
     
    singular = sy < 1e-6
 
    x = jp.where(singular, jp.arctan2(-R[1,2], R[1,1]), jp.arctan2(R[2,1] , R[2,2]))
    y = jp.where(singular, jp.arctan2(-R[2,0], sy), jp.arctan2(-R[2,0], sy))
    z = jp.where(singular, 0, jp.arctan2(R[1,0], R[0,0]))

    # if not singular :
    #     x = jp.atan2(R[2,1] , R[2,2])
    #     y = jp.atan2(-R[2,0], sy)
    #     z = jp.atan2(R[1,0], R[0,0])
    # else :
    #     x = jp.atan2(-R[1,2], R[1,1])
    #     y = jp.atan2(-R[2,0], sy)
    #     z = 0
